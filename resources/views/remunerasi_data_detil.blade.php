...
@section('title','Rincian Data Remunerasi')
  ...
  <div class="navbar-inner">    
    <div class="row-fluid">
      <div class="span12" style="display: inline-flex;">
        <form class="form-inline" method="GET" action="{{ route('remunerasi_data_detil') }}" style="margin-top: 5px; margin-bottom: 0;">
        @csrf
        ...
        <div class="btn-group" style="margin-left: 5px;">
          ...
          <div class="btn-group" style="margin-top: 0;">
            ...
            <ul class="dropdown-menu pull-right">
              <li>
                <a href="javascript:;" onclick="document.getElementById('rincian').submit();" title="Rincian Jasa">
                  Rincian Jasa
                </a>
              </li>
              <li class="divider"></li>
              <li><a href="javascript:;" onclick="document.getElementById('cetak').submit();">Cetak Remunerasi</a></li>
              @if($remun->stat == 6)
              <li><a href="{{ route('remunerasi_cetak_jaspel',Crypt::encrypt($remun->id)) }}" target="_blank">Cetak SPJ</a></li>
              <li><a href="{{ route('remunerasi_cetak_kwitansi',Crypt::encrypt($remun->id)) }}" target="_blank">Cetak Kwitansi</a></li>
              @endif
              <li class="divider"></li>
              <li><a href="{{ route('remunerasi_export',Crypt::encrypt($remun->id)) }}">Export Remunerasi</a></li>
              @if($remun->stat == 6)
              <li><a href="{{ route('remunerasi_export_jaspel',Crypt::encrypt($remun->id)) }}">Export SPJ</a></li>
              <li><a href="{{ route('remunerasi_kwitansi_export',Crypt::encrypt($remun->id)) }}">Export Kwitansi</a></li>
              @endif
            </ul>
          </div>
        </div>

        <form hidden method="GET" action="{{ route('remunerasi_data') }}" id="kembali">
        @csrf
        </form>

        <form hidden method="GET" action="{{ route('remunerasi_rincian') }}" id="rincian" target="_blank">
        @csrf
