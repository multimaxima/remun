...
<div class="navbar">
  <div class="navbar-inner">    
    <div class="row-fluid">
      <div class="span12">
        <div class="btn-group">
          <button type="submit" class="btn btn-primary" title="Kembali" form="kembali">
            KEMBALI
          </button>      
          <a href="{{ route('remunerasi_rincian_export',Crypt::encrypt($detil->id)) }}" class="btn btn-primary" title="Export">
            EXPORT
          </a>      
          <a href="{{ route('jasa_remun_cetak',Crypt::encrypt($detil->id)) }}" class="btn btn-primary" target="_blank" title="Cetak">
            CETAK
          </a>

          <form hidden method="GET" action="{{ route('jasa_remun') }}" id="kembali">
          @csrf
          </form>
        </div>
      </div>
