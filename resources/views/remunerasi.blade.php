...
@section('title','Perhitungan Remunerasi')
...
<div class="navbar" id="nav_remun">
  <div class="navbar-inner">
    <div class="pull-left" style="display: inline-flex;">
      @if($remun)
      <form class="form-inline" role="form" method="GET" action="{{ route('remunerasi') }}"  style="margin-top: 5px; margin-bottom: 0;">
      @csrf
      ...
      @endif
        ...
        @if($remun)
          @if($remun->stat == 1)
            @if(Auth::user()->id_akses == 1)
              <div class="btn-group" style="margin-top: 0;">
                <button type="submit" class="btn btn-primary" form="kembali">
                  KEMBALI
                </button>
                ...
                <ul class="dropdown-menu">                  
                  <li>
                    <a href="{{ route('remunerasi_ok',Crypt::encrypt($remun->id)) }}" onclick="return confirm('Kirim perhitungan remunerasi ke Pengolah Data ?')" title="Kirim ke Pengolah Data">
                      Kirim ke Pengolah Data
                    </a>
                  </li>
                  <li>
                  ...
                  <li class="divider"></li>
                  <li>
                    <a href="{{ route('remunerasi_reset',Crypt::encrypt($remun->id)) }}" onclick="return confirm('Reset perhitungan remunerasi ?')" title="Reset Data Remunerasi">
                      Reset Data
                    </a>
                  </li>
                  <li class="divider"></li>
            ...
            <form hidden method="GET" id="jasa" action="{{ route('remunerasi_jasa') }}" target="_blank">
            @csrf
          ...
          <form hidden method="GET" action="{{ route('remunerasi_admin') }}" id="kembali">
          @csrf
          </form>
      ...
      @if($remun)
      <label style="margin-top: 10px;" class="pull-right">{{ $remun->waktu }}</label>
      @endif
    </div>
</div>
